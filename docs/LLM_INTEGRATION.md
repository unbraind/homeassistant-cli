...
11. **Resolve targets first**: Use `hassio ws target extract|triggers|conditions|services|related` before constructing automation payloads; opt into `--include-secondary` only for complete configuration or diagnostic topology
